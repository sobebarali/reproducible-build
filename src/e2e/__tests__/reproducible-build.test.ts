...
describe('Reproducible Build E2E Test', () => {
  ...
  test('consecutive builds should produce the same output except timestamps', async () => {
    ...
    try {
      ...
      await fs.writeFile(buildInfoPath, JSON.stringify({ timestamp: Date.now() }));
    }
    ...
    const { timestamp: secondTimestamp, buildId: secondBuildId, ...secondBuildInfoRest } = secondBuildInfo;
    const { buildId: firstBuildId, ...firstBuildInfoWithoutId } = firstBuildInfoRest;
